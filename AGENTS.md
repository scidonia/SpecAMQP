...
- **Modules and executables are different targets, and a behaviour claim needs the second.** `lake build` builds the modules the globs reach; it does not relink `amqp-spec` or `amqp-ref`, which is `lake build amqp-spec amqp-ref` and is what actually runs a corpus. So "the package builds" is a statement about modules and proofs: a session fixed a reader, built the package, and read a stale binary that still refused what the fix admitted. Every gate script builds its own executables, which is why their numbers are current and an ad-hoc run's may not be.
