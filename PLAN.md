...
**A contract authored by reading the implementation inherits its errors, and three instances this session make it a class rather than an accident.** The pattern is that the contract agrees with the code *by construction*, so the check it was supposed to provide is spent before it is written:

* A generator's `refused()` helper defaulted every refusal to `framing-error`, so the corpus encoded the wrong condition for every state refusal — which is why 150 step verdicts and a differential over two artefacts all reported agreement while both were wrong. The corpus could not have caught it because the corpus *was* the error.
* `slice-sasl-challenge-from-client`'s fourth step pinned the condition the implementation happened to produce rather than the one the artifact defines, and the slice's owner found it by fixing the implementation and watching a contract disagree.
* The frame corpus's encode vector expected a non-canonical form, derived from what the writer did rather than from the table's narrowest-form rule, and was caught only when the coder read the canonicality claim instead.

The rule that follows is not "be careful": it is that when a vector or a contract is authored, the artifact is the source and the implementation is a suspect — and that a refusal or an encoding copied from observed behaviour has no evidential value at all, because it will agree with the code no matter what the code does. The checks that caught all three were independent of the thing they checked: the audit compared the artifact's definitions against the code, the register compared the artifact's definitions against a chosen condition, and the coder compared the table's rule against the writer's output.
...
What remains in the dispatch is now short: the compound and described reading-off steps (their arithmetic is already proved), and the `readValue`/`decodeValue`-level inversion tying an accepted buffer's first octet to the row and carrying `c'.pos - c.pos` up to `consumed = bytes.size`, with the array case needing `Spec.Codec.elementDecl?`'s lookup through `except_map_ok`.
