...
Remaining in S1: the round-trip and narrowest-form proofs (`Contracts/Codec.lean` states them; the proof is S1.5), dispositions for the Part 1 clauses the new vectors cite, and the zero-width array fix above.

**Open integration item, pre-existing and not gating.** `cd lean && lake build` with no target fails at job computation:

```
✖ [0/5] Running job computation
error: Generated: some modules have bad imports      (likewise Spec, Contracts, Harness, Ref)
```

Every contract and acceptance command builds *specific* targets — module targets (`Contracts.TypeSystem`, `Generated.Oasis.Types`) and executables (`amqp-ref`, `amqp-spec`) — and those are green (23 jobs), so nothing in the plan depends on the bare package build. Ruled out so far: unresolvable imports (every import in our sixteen modules names a module that exists), a missing mathlib dependency (adding it changed nothing), and a `deps` field on `lean_lib` (not a `LeanLibConfig` field in this Lake version — that experiment is reverted; it only warned). The remaining hypothesis is that one `lean_lib` per layer is the wrong shape for this Lake version and the layers should be a single library with `globs`, or libs given explicit `roots`. Assigned to the next build-infrastructure slice; recorded here because a repository whose "build everything" is broken is a repository where that breakage becomes load-bearing later.

Three gaps the reference slice opened carry forward, recorded rather than folklore:

* **Arrays of compound elements are legal and unimplemented.** So are the other legal-but-unread element constructors: `smalluint`/`smallulong`, `uint0`/`ulong0`, `list0`, boolean, short/int/long, the wide variable-width forms and `described` as element data. No vector covers any of them, and all now report `unsupported` rather than claiming the grammar is silent. Follow-up: a corpus family per class, then implementation in both artefacts.
* **A map with two identical keys is invalid** (Part 1, stated without a keyword — the sentence that forced the unkeyed capture, §6). The reader accepts it today and no vector covers it: `deferred:map-duplicate-keys` in the ledger names the follow-up, which needs key equality — structural for compound keys — and so belongs with the value-equality work rather than in the pair loop.
* **A contract can only assert an observable that the harness surfaces.** The reason-class check was written against a detail the harness did not produce: `Harness.Runner`'s reject branch discarded the codec's error and rendered `rejected with amqp:decode-error` instead, so every refusal looked alike and the check would have failed on every reject vector — not because an artefact was wrong, but because the observable had been thrown away one layer below the contract. The fix is one line in the harness (render the codec's message as the detail) and it is the general shape of the mistake: when a contract pins the *format* of something, the wiring that carries it to the boundary is part of the contract, and finding that out by a red gate is the gate working.
* **A refusal must name its reason, and both artefacts must name the same one.** Agreeing *that* a vector is refused is half a claim: two artefacts can refuse for different reasons — one because the buffer ends, the other because a count exceeds a limit — and a status-only comparison calls that agreement. The detail format is therefore `<class>: <prose>` over the named classes `truncated`, `unassigned`, `unsupported`, `sizeMismatch`, `malformed` and `limit`; the differential contract refuses a rejection nobody can name, refuses two artefacts naming different classes, and enforces the class where a vector pins `expectError.reason`. The classes are the implementation's taxonomy, not an invented one: the first five are the reference's `DecodeError` variants, and `limit` is the sixth this milestone adds.
* **`char` is framing.** Any four octets decode to a code point; the codec does not reject surrogates or values above 0x10FFFF, because the artifact fixes the framing (UTF-32BE, four octets) and leaves the code point's validity to the value domain, where the corpus range-checks it. Recorded so the eventual range check is placed deliberately at the value layer rather than silently assumed at the codec.

**The codec's laws are stated, and their proofs are the next milestone's first task.** `lean/Contracts/Codec.lean` states `RoundTripOnEncodedValues` (every value the writer encodes is read back byte for byte, consuming the whole buffer — the consumed count matters as much as the value, since a payload boundary is exactly that) and `NarrowestEncoding` (for any buffer that decodes in full, the writer's encoding of that value is no longer) as propositions, with the acceptance theorem deliberately *absent* rather than stubbed: a `def … : Prop` is a contract anybody can read, where a `theorem … := by sorry` would be a claim nobody proved and a `sorry` marker for the trust scan to catch. Their proofs are a weight-recursion argument over a nested inductive type (a value contains lists of values), not a two-line lemma, so S1's acceptance rests on the corpus and the differential contract, and the proofs are scheduled as the first work after it — with the plan saying so rather than the file implying otherwise.

Acceptance: both executables built; the corpora replayed by each artefact through the harness; the differential contract green over both corpora with its non-vacuity check satisfied (rejections present and rejected by both); vector ids conforming to the schema's pattern; the trust scan clean over the handwritten and executable modules; the ABNF/table agreement checked; ledger coverage complete for Part 1's normative clauses, including the nine keyword-free statements. The round-trip and narrowest-form *proofs* are S1.5, stated here and not claimed: until they land, the evidence for the codec's laws is the corpus (67,124 vectors, both directions, with canonical re-encode) and the differential agreement with the independently written reference.
...
Vectors, in `vectors/frames.ndjson` and `vectors/frames-negative.ndjson`: the five frame types, a frame with a zero-width extended header (`DOFF = 2`) and one with a non-multiple-of-four body offset rejected, an oversize frame against a negotiated limit, an undersize one against `MIN-MAX-FRAME-SIZE`, every one of the 40 descriptors encoded from the generated table, each descriptor's mandatory-field omission as a reject, each optional field's default as an accepted encode/decode pair, and the descriptor-code mutation control (a vector whose descriptor octet is perturbed must be refused). The mutation control is the one that catches a codec that ignores the descriptor entirely.

Acceptance: frame and performative codecs proved against the generated tables — at minimum the header arithmetic (`DOFF`, `SIZE`) and the descriptor lookup, which are the two places a hand-written layout drifts; every descriptor exercised in both directions; every negative vector rejected with its named reason class; the descriptor-code mutation control fires; and the differential contract extended to frames, since the reference's frames layer is written independently of the specification's.
