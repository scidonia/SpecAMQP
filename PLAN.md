...
What S1 deliberately does *not* claim, so nothing reads as stronger than it is: the round-trip and narrowest-form laws are stated in `Contracts/Codec.lean` and unproved (S1.5), and the reference does not read element constructors the specification reads (the gap below, scheduled after S2). Its evidence for the codec's laws is the corpus and the differential agreement between two independent derivations — which is strong, and is not a proof, and the plan says which is which.
