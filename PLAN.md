...
Remaining in S1: the round-trip and narrowest-form proofs (`Contracts/Codec.lean` states them; the proof is S1.5), dispositions for the Part 1 clauses the new vectors cite, the write-side limit in the reference and the encode-refusal path through the harness (in flight), and the element-constructor gap below.
